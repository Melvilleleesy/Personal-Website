// Forest Sea Configuration
const forestSeaTileSheet = new Image();
forestSeaTileSheet.src = 'static/images/sea-forest-tileset.png';

const forestTileSheet = new Image();
forestTileSheet.src = 'static/images/forest-tileset.png';

const FOREST_TILE_REFERENCE = {
...
let forestCacheCtx = null;
let forestCacheReady = false;
...
const FOREST_MAP = {
    ...
    height: 2304, // 36
    transitions: [
        { type: 'edge', side: 'up', y: 40, xStart: 850, xEnd: 950, goTo: 'HUB', startX: 1120, startY: 2175 }
    ],
    
    drawBackground: function(ctx, camera) {
        // Draw Sea Background
        SEA_BACKGROUND.draw(forestSeaTileSheet, ctx, camera);
        
        if (!forestBackgroundCache) {
            const cacheData = CACHE.initializeBackgroundCache(36, 36);
            forestBackgroundCache = cacheData.backgroundCache;
            forestCacheCtx = cacheData.cacheCtx;
        }
        if (!forestCacheReady && forestTileSheet.complete) {
            if (CACHE.renderBackgroundCache(forestTileSheet, "FOREST", FOREST_TILEMAP, FOREST_TILE_REFERENCE, forestBackgroundCache, forestCacheCtx, 36, 36)) {
                forestCacheReady = true;
            }
        }
    ...
    drawFromCache: function(ctx, camera) {
        ...
        ctx.drawImage(
            forestBackgroundCache,           // Source: our pre-rendered cache
